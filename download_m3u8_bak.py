...
class DownloatTsFilm:
    ...
    def _download(self, mp4_file_name: str, m3u8_url: str = "", m3u8_file_path: str = None):
        ...
        # m3u8前缀
        # prefix_m3u8 = "/".join(m3u8_url.split("index.m3u8")[0].split("/")[:-1])
        ...
        #         ts_url_list.append(i)
        #     else:
        #         ts_url_list.append(m3u8_url.replace("index.m3u8", i.split("/")[-1]))

        ts_url_list = [i.strip()
                       for i in m3u8_info if not i.startswith("#") and i != '']
        for i in range(len(ts_url_list)):
            if not ts_url_list[i].startswith("http"):
                ts_url_list[i] = m3u8_url.replace(
                    postfix_m3u8, ts_url_list[i].split("/")[-1])
                # ts_url_list[i] = os.path.join(prefix_m3u8, ts_url_list[i])
...
def test():
    ...
    m3u8_url = "https://d2hjaxnycbnwqa.cloudfront.net/20200305/enZ54Cuk/1000kb/hls/index.m3u8"
    r = requests.get(m3u8_url, headers=headers, verify=False).content
    m3u8_info = str(r, encoding="gbk").split("\n")
    print(m3u8_info)
