...
class DownloatTsFilm:
    ...
    def _download(self, mp4_file_name: str, end_ts_url: str = "", m3u8_file_path: str = None):
        """下载ts文件转mp4.

        Args:
            end_ts_url: 最后一个ts url地址. eg: "https://a5a76a73.bbq7woevaejr1zeh888.com/2022a/202211/08/63676b075424162010349f10/69f1d7/index183.ts"
        ...
        # "https://a5a76a73.bbq7woevaejr1zeh888.com/2022a/202211/08/63676b075424162010349f10/69f1d7/index183.ts"

        # 文件是否加密
        cryptor = None


        last_num = int(end_ts_url.split("index")[-1].replace(".ts", ""))
        ts_url_list = []
        for i in range(last_num + 1):
            ts_url = end_ts_url.replace(f"index{last_num}.ts", f"index{i}.ts")
    ...
    def batch_download(self):
        ...
        file_name = ""
        for m3u8_url_file_name in self._m3u8_url_file_name_list:
    ...
    def download_by_m3u8_file(self) -> None:
        ...
        end_ts_url = func_params["end_ts_url"]
        self._download(mp4_file_name=mp4_file_name,
                       end_ts_url=end_ts_url)
...
def test():
    ...
    m3u8_url = "https://cd11-ccd1-2.play.bokecc.com/flvs/C19E12B3564FE2AD/2022-09-21/7B1976CA58F3719B13358DE27D943A99-30.m3u8?t=1679771688&key=15A587837446B8D8B45FC949F31ABABD&tpl=10&tpt=231"
...
"""
python video_download/download_m3u8_zwjm.py
"""
